...
export class AppDesayunoComponent implements OnInit {
  constructor(private api: ApiService, public dialog: MatDialog, private title: Title) { }
  Name: string = ""
  Email: string = ""
  Condicion: boolean = false
  Response!: any
  ngOnInit(): void { this.title.setTitle("E-book gratuito - Todo sobre el Desayuno") }

  sendGuia() {
    if (this.Condicion) {
      if (this.Email && this.Name) {
        this.Response = { ok: true }
        this.api.SendRecursos(this.Email, this.Name, "desayuno")
      } else this.Response = { msg: "Introduce tu nombre y tu correo!" }
    } else this.Response = { msg: "Acepte los términos y condiciones!" }
    this.dialog.open(DialogDesayuno, {
